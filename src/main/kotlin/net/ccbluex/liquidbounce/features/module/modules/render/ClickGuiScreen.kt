...
class ClickGuiScreen : Screen(Component.literal("Vape ClickGUI")) {
    ...
    private fun fillRect(poseStack: PoseStack, x1: Int, y1: Int, x2: Int, y2: Int, color: Int) {
        if (x2 <= x1 || y2 <= y1) return
        DrawableHelper.fill(poseStack, x1, y1, x2, y2, color)
    }

    private fun drawRoundedRect(poseStack: PoseStack, x: Float, y: Float, w: Float, h: Float, radius: Float, color: Int) {
        val r = radius.coerceAtMost(w / 2f).coerceAtMost(h / 2f)
        val x1 = x; val y1 = y; val x2 = x + w; val y2 = y + h

        fillRect(poseStack, (x1 + r).toInt(), y1.toInt(), (x2 - r).toInt(), y2.toInt(), color)
        fillRect(poseStack, x1.toInt(), (y1 + r).toInt(), (x1 + r).toInt(), (y2 - r).toInt(), color)
        fillRect(poseStack, (x2 - r).toInt(), (y1 + r).toInt(), x2.toInt(), (y2 - r).toInt(), color)

        drawCorner(poseStack, x1 + r, y1 + r, r, 180f, 270f, color)
        drawCorner(poseStack, x2 - r, y1 + r, r, 270f, 360f, color)
        drawCorner(poseStack, x2 - r, y2 - r, r, 0f, 90f, color)
        drawCorner(poseStack, x1 + r, y2 - r, r, 90f, 180f, color)
    }

    private fun drawCorner(poseStack: PoseStack, cx: Float, cy: Float, r: Float, start: Float, end: Float, color: Int) {
        var a = start
        while (a < end) {
            ...
            fillRect(poseStack, minX, minY, max(minX + 1, maxX), max(minY + 1, maxY), color)
            a += 8f
        }
    }

    private fun drawVapeShadow(poseStack: PoseStack, x: Float, y: Float, w: Float, h: Float) {
        for (i in 0..8) {
            val offset = i.toFloat()
            val alpha = (15 * (1f - i / 8f)).toInt()
            drawRoundedRect(poseStack, x - offset, y - offset, w + offset * 2, h + offset * 2, CORNER + offset, (alpha shl 24) or 0x000000)
        }
    }
    ...
    override fun render(poseStack: PoseStack, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        // 窗口阴影
        drawVapeShadow(poseStack, winX, winY, WIN_W.toFloat(), WIN_H.toFloat())
        // 窗口背景
        drawRoundedRect(poseStack, winX, winY, WIN_W.toFloat(), WIN_H.toFloat(), CORNER, BG)

        // ===== 标题栏 =====
        drawRoundedRect(poseStack, winX, winY, WIN_W.toFloat(), HEADER_H.toFloat(), CORNER, HEADER)
        font.draw(poseStack, Component.literal("§lVape §f§lV5"), winX + 12f, winY + 6f, ACCENT)
        font.draw(poseStack, Component.literal("§7v5.0"), winX + WIN_W - 50f, winY + 7f, TEXT_DIM)
        ...
        drawRoundedRect(poseStack, searchX, searchY, searchW.toFloat(), SEARCH_H.toFloat(), 6f, 0x28FFFFFF.toInt())
        font.draw(poseStack, Component.literal("🔍"), searchX + 6f, searchY + 6f, TEXT_DIM)

        val displayText = if (searchText.isEmpty()) "§7搜索功能..." else "§f$searchText"
        font.draw(poseStack, Component.literal(trimText(font, displayText, searchW - 30)), searchX + 22f, searchY + 6f, TEXT)
        ...
        if (searchFocused) {
            ...
            if (cursorX < searchX + searchW - 4) {
                ...
                if (blink) fillRect(poseStack, cursorX, searchY.toInt() + 4, cursorX + 1, searchY.toInt() + SEARCH_H - 4, TEXT_BRIGHT)
            }
        }
        ...
        fillRect(poseStack, winX.toInt() + 4, tabY.toInt(), winX.toInt() + WIN_W - 4, tabY.toInt() + TAB_H.toInt(), 0x18000000.toInt())
        ...
        categories.forEachIndexed { i, cat ->
            ...
            if (isActive) {
                fillRect(poseStack, tx.toInt(), tabY.toInt(), (tx + tabW - 2).toInt(), (tabY + TAB_H).toInt(), ACCENT)
                fillRect(poseStack, tx.toInt(), (tabY + TAB_H - 2).toInt(), (tx + tabW - 2).toInt(), (tabY + TAB_H).toInt(), ACCENT_DARK)
            } else if (mouseX in tx.toInt()..(tx + tabW - 2).toInt() && mouseY in tabY.toInt()..(tabY + TAB_H).toInt()) {
                fillRect(poseStack, tx.toInt(), tabY.toInt(), (tx + tabW - 2).toInt(), (tabY + TAB_H).toInt(), HOVER)
            }

            val label = trimText(font, cat.tag.uppercase(), tabW - 4)
            val cw = font.width(label)
            font.draw(poseStack, Component.literal(label), tx + ((tabW - 2) - cw) / 2f, tabY + 4f, if (isActive) TEXT_BRIGHT else TEXT_DIM)
        }

        // ===== 分割线 =====
        val divY = tabY + TAB_H + 1
        fillRect(poseStack, winX.toInt() + 8, divY.toInt(), winX.toInt() + WIN_W - 8, divY.toInt() + 1, BORDER)
        ...
        modules.forEachIndexed { i, mod ->
            ...
            if (isHover) fillRect(poseStack, winX.toInt() + 6, yPos.toInt(), listRight.toInt(), yPos.toInt() + ITEM_H.toInt(), HOVER)
            if (isExpanded) {
                fillRect(poseStack, winX.toInt() + 6, yPos.toInt(), listRight.toInt(), yPos.toInt() + ITEM_H.toInt(), Color(0x00FF9D, 15).rgb)
            }

            val nameColor = if (mod.enabled) ACCENT else TEXT_DIM
            val nameText = if (isExpanded) "§n${mod.name}" else mod.name
            font.draw(poseStack, Component.literal(trimText(font, nameText, (listRight - winX - 40).toInt())), winX + 14f, yPos + 5f, nameColor)
            ...
            if (mod.enabled) {
                drawRoundedRect(poseStack, toggleX.toFloat(), toggleY.toFloat(), toggleW.toFloat(), toggleH.toFloat(), 6f, ACCENT)
                fillRect(poseStack, toggleX + toggleW - 8, toggleY + 2, toggleX + toggleW - 2, toggleY + toggleH - 2, TEXT_BRIGHT)
                fillRect(poseStack, toggleX + toggleW - 8, toggleY + 2, toggleX + toggleW - 2, toggleY + toggleH - 2, Color(0x00FF9D, 40).rgb)
            } else {
                drawRoundedRect(poseStack, toggleX.toFloat(), toggleY.toFloat(), toggleW.toFloat(), toggleH.toFloat(), 6f, 0x30FFFFFF.toInt())
                fillRect(poseStack, toggleX + 2, toggleY + 2, toggleX + 8, toggleY + toggleH - 2, 0xAA808080.toInt())
            }
        }
        ...
        drawRoundedRect(poseStack, detailX, bodyY, detailW, bodyH.toFloat(), 6f, PANEL)

        if (expandedModule != null) {
            renderModuleDetail(poseStack, expandedModule!!, detailX, bodyY, detailW, bodyH.toFloat(), mouseX, mouseY)
        } else {
            font.draw(poseStack, Component.literal("§7选择一个模块配置"), detailX + 8f, bodyY + 8f, TEXT_DIM)
        }
    ...
    private fun renderModuleDetail(
        ...
        // 标题
        font.draw(poseStack, Component.literal("§l${mod.name}"), x + 8f, y + 4f, if (mod.enabled) ACCENT else TEXT)
        val statusText = if (mod.enabled) "§a● 已启用" else "§c● 已禁用"
        font.draw(poseStack, Component.literal(statusText), x + 8f, y + 18f, TEXT_DIM)

        // 分割线
        fillRect(poseStack, x.toInt() + 6, y.toInt() + 32, x.toInt() + w - 6, y.toInt() + 33, BORDER)
        ...
        visibleValues.forEach { (v, depth) ->
            ...
            if (curY >= listY - itemHeight && curY <= listY + listH + itemHeight) {
                ...
                if (isHover) fillRect(poseStack, x.toInt() + 4, yPos, (x + w - 4).toInt(), yPos + itemHeight.toInt(), HOVER)
                ...
                if (isGroup) {
                    ...
                    font.draw(poseStack, Component.literal(groupName), x + 8 + indent, yPos + 2f, TEXT)
                    if (isHover) fillRect(poseStack, x.toInt() + 4, yPos, (x + w - 4).toInt(), yPos + 1, ACCENT)
                } else {
                    renderParamValue(poseStack, v, x, yPos, w, indent, mouseX, mouseY)
                }
            }
            curY += itemHeight
        }
    ...
    private fun renderParamValue(
        ...
        when {
            actual is Boolean -> {
                ...
                font.draw(poseStack, Component.literal(text), labelX.toFloat(), y + 3f, TEXT_DIM)
                val status = if (actual) "§aON" else "§cOFF"
                font.draw(poseStack, Component.literal(status), valueX.toFloat(), y + 3f, if (actual) ACCENT else TEXT_DIM)
            }
            isBind -> {
                ...
                font.draw(poseStack, Component.literal(display), valueX.toFloat(), y + 3f, if (isListening) ACCENT else TEXT_DIM)
            }
            isSlider -> {
                val text = trimText(font, v.name, (w - 80 - indent).toInt())
                font.draw(poseStack, Component.literal(text), labelX.toFloat(), y + 2f, TEXT_DIM)
                ...
                fillRect(poseStack, sliderX, sliderY, sliderX + sliderW, sliderY + 3, 0x30FFFFFF.toInt())
                fillRect(poseStack, sliderX, sliderY, sliderX + (sliderW * progress).toInt(), sliderY + 3, ACCENT)
                fillRect(poseStack, sliderX + (sliderW * progress).toInt() - 2, sliderY - 1, sliderX + (sliderW * progress).toInt() + 2, sliderY + 4, TEXT_BRIGHT)

                val valueStr = if (actual is ClosedRange<*>) "${actual.start}-${actual.endInclusive}" else "%.1f".format(fv)
                font.draw(poseStack, Component.literal(valueStr), sliderX + sliderW + 4f, y + 1f, TEXT_DIM)
            }
            isColor -> {
                val text = trimText(font, v.name, (w - 60 - indent).toInt())
                font.draw(poseStack, Component.literal(text), labelX.toFloat(), y + 3f, TEXT_DIM)
                val color = extractColor(v)
                fillRect(poseStack, valueX, y + 2, valueX + 14, y + 16, color.rgb)
                fillRect(poseStack, valueX, y + 2, valueX + 14, y + 16, BORDER)
            }
            else -> {
                val display = getDisplayValue(v)
                val text = trimText(font, v.name, (w - 60 - indent).toInt())
                font.draw(poseStack, Component.literal(text), labelX.toFloat(), y + 3f, TEXT_DIM)
                font.draw(poseStack, Component.literal("§7$display"), valueX.toFloat(), y + 3f, TEXT_DIM)
            }
        }
    }
