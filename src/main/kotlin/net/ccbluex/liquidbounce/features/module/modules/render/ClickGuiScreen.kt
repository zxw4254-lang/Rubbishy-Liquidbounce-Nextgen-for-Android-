...
class ClickGuiScreen : Screen(Component.literal("Vape ClickGUI")) {
    ...
    override fun render(matrices: MatrixStack, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        font.draw(matrices, Component.literal("§lVape §f§lV5"), winX + 12f, winY + 6f, ACCENT)
        font.draw(matrices, Component.literal("§7v5.0"), winX + WIN_W - 50f, winY + 7f, TEXT_DIM)
        ...
        font.draw(matrices, Component.literal("🔍"), searchX + 6f, searchY + 6f, TEXT_DIM)

        val displayText = if (searchText.isEmpty()) "§7搜索功能..." else "§f$searchText"
        font.draw(matrices, Component.literal(trimText(font, displayText, searchW - 30)), searchX + 22f, searchY + 6f, TEXT)
        ...
        categories.forEachIndexed { i, cat ->
            ...
            font.draw(matrices, Component.literal(label), tx + ((tabW - 2) - cw) / 2f, tabY + 4f, if (isActive) TEXT_BRIGHT else TEXT_DIM)
        }
        ...
        modules.forEachIndexed { i, mod ->
            ...
            font.draw(matrices, Component.literal(trimText(font, nameText, (listRight - winX - 40).toInt())), winX + 14f, yPos + 5f, nameColor)
        ...
        if (expandedModule != null) {
            ...
            font.draw(matrices, Component.literal("§7选择一个模块配置"), detailX + 8f, bodyY + 8f, TEXT_DIM)
        }
    ...
    private fun renderModuleDetail(
        ...
        // 标题
        font.draw(matrices, Component.literal("§l${mod.name}"), x + 8f, y + 4f, if (mod.enabled) ACCENT else TEXT)
        val statusText = if (mod.enabled) "§a● 已启用" else "§c● 已禁用"
        font.draw(matrices, Component.literal(statusText), x + 8f, y + 18f, TEXT_DIM)
        ...
        visibleValues.forEach { (v, depth) ->
            ...
            if (curY >= listY - itemHeight && curY <= listY + listH + itemHeight) {
                ...
                if (isGroup) {
                    ...
                    font.draw(matrices, Component.literal(groupName), x + 8 + indent, yPos + 2f, TEXT)
    ...
    private fun renderParamValue(
        ...
        when {
            actual is Boolean -> {
                ...
                font.draw(matrices, Component.literal(text), labelX.toFloat(), y + 3f, TEXT_DIM)
                val status = if (actual) "§aON" else "§cOFF"
                font.draw(matrices, Component.literal(status), valueX.toFloat(), y + 3f, if (actual) ACCENT else TEXT_DIM)
            }
            isBind -> {
                ...
                font.draw(matrices, Component.literal(display), valueX.toFloat(), y + 3f, if (isListening) ACCENT else TEXT_DIM)
            }
            isSlider -> {
                val text = trimText(font, v.name, (w - 80 - indent).toInt())
                font.draw(matrices, Component.literal(text), labelX.toFloat(), y + 2f, TEXT_DIM)
                ...
                font.draw(matrices, Component.literal(valueStr), sliderX + sliderW + 4f, y + 1f, TEXT_DIM)
            }
            isColor -> {
                val text = trimText(font, v.name, (w - 60 - indent).toInt())
                font.draw(matrices, Component.literal(text), labelX.toFloat(), y + 3f, TEXT_DIM)
                ...
                val text = trimText(font, v.name, (w - 60 - indent).toInt())
                font.draw(matrices, Component.literal(text), labelX.toFloat(), y + 3f, TEXT_DIM)
                font.draw(matrices, Component.literal("§7$display"), valueX.toFloat(), y + 3f, TEXT_DIM)
            }
        }
    }
