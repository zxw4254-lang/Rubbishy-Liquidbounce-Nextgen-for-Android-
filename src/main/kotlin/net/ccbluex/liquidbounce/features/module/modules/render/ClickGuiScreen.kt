...
class ClickGuiScreen : Screen(Component.literal("Vape ClickGUI")) {
    ...
    override fun render(poseStack: PoseStack, mouseX: Int, mouseY: Int, delta: Float) {
        ...
        font.draw(poseStack, "§lVape §f§lV5", winX + 12f, winY + 6f, ACCENT)
        font.draw(poseStack, "§7v5.0", winX + WIN_W - 50f, winY + 7f, TEXT_DIM)
        ...
        font.draw(poseStack, "🔍", searchX + 6f, searchY + 6f, TEXT_DIM)

        val displayText = if (searchText.isEmpty()) "§7搜索功能..." else "§f$searchText"
        font.draw(poseStack, trimText(font, displayText, searchW - 30), searchX + 22f, searchY + 6f, TEXT)
        ...
        categories.forEachIndexed { i, cat ->
            ...
            font.draw(poseStack, label, tx + ((tabW - 2) - cw) / 2f, tabY + 4f, if (isActive) TEXT_BRIGHT else TEXT_DIM)
        }
        ...
        modules.forEachIndexed { i, mod ->
            ...
            font.draw(poseStack, trimText(font, nameText, (listRight - winX - 40).toInt()), winX + 14f, yPos + 5f, nameColor)
        ...
        if (expandedModule != null) {
            ...
            font.draw(poseStack, "§7选择一个模块配置", detailX + 8f, bodyY + 8f, TEXT_DIM)
        }
    ...
    private fun renderModuleDetail(
        ...
        // 标题
        font.draw(poseStack, "§l${mod.name}", x + 8f, y + 4f, if (mod.enabled) ACCENT else TEXT)
        val statusText = if (mod.enabled) "§a● 已启用" else "§c● 已禁用"
        font.draw(poseStack, statusText, x + 8f, y + 18f, TEXT_DIM)
        ...
        visibleValues.forEach { (v, depth) ->
            ...
            if (curY >= listY - itemHeight && curY <= listY + listH + itemHeight) {
                ...
                if (isGroup) {
                    ...
                    font.draw(poseStack, groupName, x + 8 + indent, yPos + 2f, TEXT)
    ...
    private fun renderParamValue(
        ...
        when {
            actual is Boolean -> {
                ...
                font.draw(poseStack, text, labelX.toFloat(), y + 3f, TEXT_DIM)
                val status = if (actual) "§aON" else "§cOFF"
                font.draw(poseStack, status, valueX.toFloat(), y + 3f, if (actual) ACCENT else TEXT_DIM)
            }
            isBind -> {
                ...
                font.draw(poseStack, display, valueX.toFloat(), y + 3f, if (isListening) ACCENT else TEXT_DIM)
            }
            isSlider -> {
                val text = trimText(font, v.name, (w - 80 - indent).toInt())
                font.draw(poseStack, text, labelX.toFloat(), y + 2f, TEXT_DIM)
                ...
                font.draw(poseStack, valueStr, sliderX + sliderW + 4f, y + 1f, TEXT_DIM)
            }
            isColor -> {
                val text = trimText(font, v.name, (w - 60 - indent).toInt())
                font.draw(poseStack, text, labelX.toFloat(), y + 3f, TEXT_DIM)
                ...
                val text = trimText(font, v.name, (w - 60 - indent).toInt())
                font.draw(poseStack, text, labelX.toFloat(), y + 3f, TEXT_DIM)
                font.draw(poseStack, "§7$display", valueX.toFloat(), y + 3f, TEXT_DIM)
            }
        }
    }
