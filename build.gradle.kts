...
val isAndroidBuild: Boolean = providers.gradleProperty("build.target")
    .orElse("standard")
    .get()
...
if (isAndroidBuild) {
    ...
    logger.lifecycle("Some features (JCEF, Discord IPC, DJL, WayGL, LWJGL-EGL) will be excluded.")
}
...
dependencies {
    ...
    // NOT available on Android - exclude in Android builds
    if (!isAndroidBuild) {
        jij(libs.lwjgl.egl)
        modApi(libs.waygl)
    ...
    if (!isAndroidBuild) {
        ...
        // Keep MCEF as compileOnly so code compiles, but don't bundle it
        compileOnly(libs.mcef)
        compileOnly(libs.httpServer)
        // Also keep OkHttpProgressInterceptor available as compileOnly
        // (it's part of MCEF but used in HttpClient)
        compileOnly(libs.mcef)
    }

    // Discord RPC Support - NOT available on Android (no Discord native binaries)
    if (!isAndroidBuild) {
        jij(libs.discordIpc)
    }
